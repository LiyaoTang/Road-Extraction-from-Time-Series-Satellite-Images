...
parser.add_option("--learning_rate", type="float", default=4e-6, dest="learning_rate")
...
if concat_input:
    concat_input = [[[int(x) for x in config.split('-')] for config in  layer.split(';')] for layer in concat_input.split('|')]
    ...
    if concat_input:
        if concat_input == [[[0]]]:
            ...
            if len(concat_input) > 1:
                for layer_cnt in range(len(concat_input)-1):
                    layer_cfg = concat_input[layer_cnt + 1]
...
for epoch_num in range(epoch):
    ...
    for iter_num in range(iteration):
        ...
        _, cur_xen = sess.run([train_step, cross_entropy], feed_dict={x: batch_x, y: batch_y, weight: batch_w, is_training: True})

        train_cross_entropy += cur_xen

    # average xen
    train_cross_entropy = train_cross_entropy / iteration

    # save model
    saver.save(sess, save_path + model_name, epoch_num)
