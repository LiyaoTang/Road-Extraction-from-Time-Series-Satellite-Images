...
for sub_dir in `cd ${pred_dir} && ls`; do
    ...
    for file in `cd ${pred_dir}${sub_dir} && ls *.h5 2>/dev/null`; do
        ...
        if [ ! "$(ls -A ${sub_pred_name}*CV* 2>/dev/null)" ]; then
            ...
            python Analyze_Pred.py --path ${path} --name ${file} --pred_weight ${weight} --save ${save_dir} --step ${step} --size ${size} --analyze_train
            # python Analyze_Pred.py --path ${path} --name ${file} --pred_weight ${weight} --save ${save_dir} --step ${step} --size ${size} --analyze_CV
        else
